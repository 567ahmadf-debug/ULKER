...
export default function OfferDetailPage() {
  ...
  if (!offer) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-20">
        ...
        <Link href="/offers">
          <span className="text-primary font-semibold cursor-pointer hover:underline">Back to Offers</span>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-20 pb-20 lg:pb-0" data-testid="page-offer-detail">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
          ...
          <span>/</span>
          <span className="text-foreground font-medium truncate">{offer.title}</span>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <div className="bg-card border border-card-border rounded-2xl p-6 space-y-6">
            <div>
              <h1 className="text-2xl font-black text-foreground mb-2">{offer.title}</h1>
              {offer.description && (
            ...
            <div className="border-t border-border pt-4">
              ...
              {offer.type === "bundle" && (
                <div>
                  ...
                  {offer.offerPrice && (
                    <p className="text-2xl font-black text-foreground">${offer.offerPrice} <span className="text-sm font-semibold text-muted-foreground">bundle price</span></p>
                  )}
                </div>
              )}
            </div>
            ...
            {offer.type === "bundle" && offer.bundleItems.length > 0 && (
              <div className="border-t border-border pt-4">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">Products in Bundle</p>
                <div className="space-y-2">
                  {offer.bundleItems.filter((b) => b.productName).map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 bg-muted/40 rounded-xl px-4 py-2.5">
                      ...
                      <span className="text-xs text-muted-foreground ml-auto">×{item.quantity}</span>
                    </div>
                  ))}
                </div>
              </div>
